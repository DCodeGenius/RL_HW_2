...
def value_iteration(mdp, gamma, nIt):
    ...
    for it in range(nIt):
        ...
        V = np.array([max(sum(p * (r + gamma * Vprev[ns]) for p, ns, r in mdp.P[s][a])
                          for a in range(mdp.nA))
                      for s in range(mdp.nS)])
        pi = np.array([np.argmax([sum(p * (r + gamma * Vprev[ns]) for p, ns, r in mdp.P[s][a])
                                  for a in range(mdp.nA)])
                       for s in range(mdp.nS)], dtype=int)
